...
public abstract class Control extends DynamicExtensionBaseDomainObject
		implements Serializable, ControlInterface {
	...
	/**
	 * Sequence number of the control.This governs in which order it will be
	 * shown on the UI.
	 */
	...
	/**
	 * @hibernate.property name="isCalculated" type="boolean"
	...
	public Boolean getIsSourceForCalculatedAttribute() {
		...
		if (baseAbstractAttributeInterface instanceof CategoryAttributeInterface) {
			...
			isSourceControl = categoryAttributeInterface
	...
	/**
	 * @hibernate.property name="sequenceNumber" type="integer"
	...
	public final String generateHTML(ContainerInterface container)
		...
		if (getParentContainer().getMode() != null
				&& getParentContainer().getMode().equalsIgnoreCase(
		...
		if (isSubControl) {
			...
			if (baseAbstractAttribute == null) {
				htmlString = htmlString
						.concat("<td class='formRequiredLabel_withoutBorder'>");
	...
	protected String getControlHTML(String htmlString)
			throws DynamicExtensionsSystemException {
		boolean isControlRequired = UserInterfaceiUtility
				.isControlRequired(this);
		StringBuffer controlHTML = new StringBuffer(434);
		// For category attribute controls, if heading and/or notes are
		// specified, then
		// render the UI that displays heading followed by notes for particular
		// category attribute controls.
		if (heading != null || getFormNotes() != null
				&& getFormNotes().size() != 0) {
			if (getIsSkipLogicTargetControl()) {
				controlHTML
						.append("</tr><tr><td width='100%' colspan='3' align='left'><div id='");
				...
				controlHTML.append(getHTMLComponentName());
				controlHTML
						.append("_row_div_heading' /><input type='hidden' name='skipLogicHideControls' id='skipLogicHideControls' value = '");
				controlHTML.append(getHTMLComponentName());
				controlHTML.append("_row_div_heading' />");
			} else {
				controlHTML
						.append("<tr><td width='100%' colspan='3' align='left'>");
			}

			if (heading != null && heading.length() != 0) {
				controlHTML
						.append("<div style='width:100%' class='td_color_6e81a6'>");
			...
			if (getFormNotes() != null && getFormNotes().size() != 0) {
				...
				for (FormControlNotesInterface fcNote : getFormNotes()) {
					...
					controlHTML.append(DynamicExtensionsUtility
							.replaceHTMLSpecialCharacters(fcNote.getNote()));
					controlHTML.append("</div>");
				}
			}
			if (getIsSkipLogicTargetControl()) {
		...
		if (yPosition != null && yPosition <= 1) {
			...
			controlHTML
					.append("<td class='formRequiredNotice_withoutBorder' width='2%'>");

			if (isControlRequired) {
				controlHTML.append("<span class='font_red'>");
				controlHTML.append(getParentContainer()
		...
		controlHTML
				.append("<td class='formRequiredLabel_withoutBorder' title='");
		...
		if (baseAbstractAttribute instanceof CategoryAttributeInterface
				&& ((CategoryAttributeInterface) baseAbstractAttribute)
						.getAbstractAttribute().getTaggedValue(
								XMIConstants.TAGGED_KEY_HS) != null) {
			controlHTML
					.append(((CategoryAttributeInterface) getBaseAbstractAttribute())
							.getAbstractAttribute().getTaggedValue(
									XMIConstants.TAGGED_KEY_HS));
		}
		controlHTML.append("'>");

		if (showLabel != null && showLabel) {
			controlHTML
					.append(((BaseAbstractAttribute) getBaseAbstractAttribute())
							.getCapitalizedName(DynamicExtensionsUtility
									.replaceHTMLSpecialCharacters(getCaption())));
		}
		controlHTML
				.append("</td><td class='formField_withoutBorder' valign='center'>");
	...
	public String getHTMLComponentName()
		...
		if (getSequenceNumber() != null) {
			...
			htmlComponentName.append(parentContainer.getIncontextContainer()
					.getId());
	...
	/**
	 * This method returns boolean indicating whether the control is a part of
	 * Sub-Form or not. true value indicates the control is a part of Sub-Form,
	 * false value indicates its not.
	 *
	...
	/**
	 * This method sets whether the control is a part of Sub-Form or not. true
	 * value indicates the control is a part of Sub-Form, false value indicates
	 * its not.
	 *
	 * @param isSubControl
	...
	/**
	 * @hibernate.many-to-one cascade="save-update" column="BASE_ABST_ATR_ID"
	 *                        class=
	 *                        "edu.common.dynamicextensions.domain.BaseAbstractAttribute"
	 ...
	 */
	public void setBaseAbstractAttribute(
	...
	public AttributeMetadataInterface getAttibuteMetadataInterface() {
		...
		if (baseAbstractAttribute instanceof AttributeMetadataInterface) {
			...
			attributeMetadata = (AttributeMetadataInterface) ValidatorUtil.getMultiselectAtrribute((AssociationInterface) baseAbstractAttribute);
		}
		...
		if (isSkipLogic) {
			...
			for (SkipLogicAttributeInterface skipLogicAttributeInterface : readOnlySkipLogicAttributes) {
				...
				if (targetControl.getIsShowHide()) {
					targetControl
							.setIsSkipLogicShowHideTargetControl(Boolean.TRUE);
					targetControl.setIsSkipLogicReadOnly(Boolean.TRUE);
				}
			...
			for (SkipLogicAttributeInterface skipLogicAttributeInterface : nonReadOnlySkipLogicAttributes) {
				...
				targetControl
						.setIsSkipLogicShowHideTargetControl(Boolean.FALSE);
			...
			for (ControlInterface controlInterface : controlCollection) {
				if (controlInterface.getSourceSkipControl() != null
						&& controlInterface.getSourceSkipControl()
								.getBaseAbstractAttribute().getName().equals(
										getBaseAbstractAttribute().getName())) {
	...
	/**
	 * @hibernate.property name="isSkipLogicTargetControl" type="boolean"
	...
	/**
	 * @hibernate.many-to-one column="SOURCE_CONTROL_ID"
	...
	/**
	 * @hibernate.property name="isSelectiveReadOnly" type="boolean"
	...
	public Collection<UIProperty> getControlTypeValues() {
		...
		for (ControlEnum propertyType : uiPropertyValues) {
			...
			if (controlProperty != null) {
				uiProperties.add(new UIProperty(propertyType.getValue(),
						controlProperty));
			}
		}
		return uiProperties;
	...
	public void setControlTypeValues(Collection<UIProperty> uiProperties) {
		...
		for (UIProperty uiProperty : uiProperties) {
			ControlEnum propertyType = ControlEnum
					.getValue(uiProperty.getKey());
	 ...
	 *
	 * @hibernate.property name="isPasteEnable" type="boolean"
	 ...
	 */
	public String getOnchangeServerCall()
			throws DynamicExtensionsSystemException {
	...
	private String updateServerState() throws DynamicExtensionsSystemException {
		Object contextId = getParentContainer().getContextParameter(WebUIManagerConstants.FORM_CONTEXT_IDENTIFIER);
		...
		updateServerState.append("','");
		updateServerState.append(contextId==null?"":contextId);
		updateServerState.append("');");
	...
	protected String getDefaultValueForControl() {
		...
		if (value == null || value.toString().length() == 0) {
			if (getAttibuteMetadataInterface() instanceof CategoryAttributeInterface
					&& isSkipLogicDefaultValue()) {
	 ...
	 */
	protected boolean isSkipLogicDefaultValue() {
	 ...
	 */
	protected String getDefaultSkipLogicValue() {
		CategoryAttributeInterface categoryAttribute = (CategoryAttributeInterface) getAttibuteMetadataInterface();
		return categoryAttribute.getDefaultSkipLogicValue().getValueAsObject()
				.toString();
	}
