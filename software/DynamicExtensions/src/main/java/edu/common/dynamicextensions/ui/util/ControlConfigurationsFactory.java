...
public final class ControlConfigurationsFactory
{
	...
	private void loadControlConfigurations() throws DynamicExtensionsSystemException
	{
		...
		List<NameValueBean> controlNameList = new ArrayList<NameValueBean>();
		for (Control controlNode : controlNodeList.getControl())
		{
			controlNameList.add(new NameValueBean(controlNode.getName(),
					controlNode.getDisplayLabel()));
	...
	{
		controlsConfigurationObject.setCommonExplicitRules(new ArrayList());
		controlsConfigurationObject.setCommonImplicitRules(new ArrayList());
		if (controlNode.getCommonValidation() != null)
		{
			List commonImplicitRules = controlsConfigurationObject.getCommonImplicitRules();
			List commonExplicitRules = controlsConfigurationObject.getCommonExplicitRules();
			List<String> ruleNameList = new ArrayList<String>();
	 ...
	 */
	public List<NameValueBean> getListOfControls()
	{
		ArrayList<NameValueBean> listOfControls = null;
		if (controlsConfigurationMap != null)
		{
			listOfControls = (ArrayList<NameValueBean>) controlsConfigurationMap.get("ListOfControls");
		}
