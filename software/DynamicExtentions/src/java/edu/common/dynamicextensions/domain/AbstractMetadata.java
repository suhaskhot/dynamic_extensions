...

{
	...
	{
		copyAbstractMetadata(abstractMetadata,this);
		
	}

	public void copyAbstractMetadata(AbstractMetadata abstractMetadata, AbstractMetadata copyAttribute)
	{
		copyAttribute.name = abstractMetadata.getName();
		copyAttribute.lastUpdated = abstractMetadata.getLastUpdated();
		copyAttribute.activityStatus = abstractMetadata.getActivityStatus();
		copyAttribute.createdDate = abstractMetadata.getCreatedDate();
		copyAttribute.description = abstractMetadata.getDescription();
		for(SemanticPropertyInterface semanticProperty: abstractMetadata.getSemanticPropertyCollection() )
		{
			copyAttribute.semanticPropertyCollection.add(new SemanticProperty(semanticProperty));
		}
		for(TaggedValueInterface interface1: abstractMetadata.getTaggedValueCollection())
		{
			copyAttribute.taggedValueCollection.add(new TaggedValue((TaggedValue)interface1));
		}
	}
}