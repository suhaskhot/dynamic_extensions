...
{
	private static final String CLONED_CATEGORY_NAME_SUFFIX = "_published";
	...
	public Category(Category category) throws DynamicExtensionsSystemException
	{
		super(category);
		//Attributes
		
		this.isCacheable = category.getIsCacheable();
		this.isPopulateFromXml = category.getIsPopulateFromXml();
		
		this.name = category.getName()+CLONED_CATEGORY_NAME_SUFFIX;
		this.processorClass = category.getProcessorClass();
		this.publicId = category.getPublicId();
		this.userId = category.getUserId();
		Map<String, CategoryEntity> catEntNameMap = new HashMap<String, CategoryEntity>();
		
		this.rootCategoryElement = new CategoryEntity(category.getRootCategoryElement(),catEntNameMap);
		this.rootCategoryElement.setCategory(this);
		
		
		//Associations
		for(AutoLoadXpath autoLoadXpath:category.autoLoadXpathCollection)
		{
			this.autoLoadXpathCollection.add(new AutoLoadXpath(autoLoadXpath));
		}
		if(this.layout instanceof SurveyModeLayout)
		{
			this.layout = new SurveyModeLayout((SurveyModeLayout)category.getLayout(),catEntNameMap);
		}
		for(CategoryEntityInterface entity:category.getRelatedAttributeCategoryEntityCollection())
		{
			this.relatedAttributeCategoryEntityCollection.add((CategoryEntity)new CategoryEntity(entity,catEntNameMap));
		}
