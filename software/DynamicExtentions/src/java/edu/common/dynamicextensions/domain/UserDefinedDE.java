...
public class UserDefinedDE extends DataElement implements UserDefinedDEInterface
{
	...
	public UserDefinedDE(UserDefinedDE dataElement) throws DynamicExtensionsSystemException
	{
		this.activationDate = dataElement.getActivationDate();
		this.isOrdered = dataElement.getIsOrdered();
		if(!dataElement.getDefaultPermissibleValues().isEmpty())
		{
			for(PermissibleValueInterface permissibleValue: dataElement.getDefaultPermissibleValues())
			{
				this.defaultPermissibleValues.add((PermissibleValueInterface) PermissibleValueUtility.copy(permissibleValue));
			}
		}
		if(!dataElement.getPermissibleValueCollection().isEmpty())
		{
			for(PermissibleValueInterface permissibleValue: dataElement.getPermissibleValueCollection())
			{
				this.permissibleValueCollection.add((PermissibleValueInterface) PermissibleValueUtility.copy(permissibleValue));
			}
		}
	}

	public UserDefinedDE()
	{
		super();
	}
