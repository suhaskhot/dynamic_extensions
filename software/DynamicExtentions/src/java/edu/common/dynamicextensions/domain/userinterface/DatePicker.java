...
public class DatePicker extends Control implements DatePickerInterface, ValidatableInterface,SummaryControlInterface
{
			...
			throws DynamicExtensionsSystemException
	{
		...
		outputStringBuffer.append("' value='"+defaultValue+"' onchange=\"" +getOnchangeServerCall()
				+"\" onclick=\""+ String.format(showCalendarCall, htmlComponentName,formatSpecifier,this.id,getParentContainer().getId())+"\"  size='16'/>");
	...
	public DatePicker(DatePicker control)
	{
		super(control);
		this.showCalendar = control.getShowCalendar();
		this.dateValueType = control.getDateValueType();
	}
}