			...
			DynamicExtensionsQueryBuilderConstantsInterface
{
		...
		if (noOfMany2OneAsso != 0)
		{
			...
			finally
			{
				try
				{
					...
					{
						Logger.out.info("**************DE BASE QUERY BUILDER ****************");
			...
			while (assoIter.hasNext())
			{
				...
				else
				{
					if (EntityManagerUtil.isCardinalityChanged(association, assoDbaseCopy)
							|| EntityManagerUtil.isParentPrimaryKeyChanged(entity)
							|| EntityManagerUtil.isParentPrimaryKeyChanged(association.getTargetEntity()))
					{
			...
			Stack<String> rlbkQryStack) throws DynamicExtensionsSystemException
	{
		...
		try
		{
			...
			try
			{
				Logger.out.info("DynamicExtensionBaseQueryBuilder :: executeQueries :: Suspending Transaction ");
			...
			try
			{
				Logger.out.info("DynamicExtensionBaseQueryBuilder :: executeQueries :: Resume Transaction  :: ");
