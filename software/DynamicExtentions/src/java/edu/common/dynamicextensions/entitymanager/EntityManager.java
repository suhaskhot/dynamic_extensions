...
//				substParams);
		...
		Collection<AssociationInterface> associations = EntityCache.getInstance().getAssociations(srcEntityId,tgtEntityId);
		return associations;
	}
...
//			hibernateDAO.commit();
	...
	/* (non-Javadoc)
	 * @see edu.common.dynamicextensions.entitymanager.EntityManagerInterface#getContainerByAbstractEntId(java.lang.Long)
	 */

	public ContainerInterface getContainerByAbstractEntId(Long abstractEntId)
	throws DynamicExtensionsSystemException
	{
		ContainerInterface container = null;
		...
		// Create a map of substitution parameters.
		Map<String, NamedQueryParam> substParams = new HashMap<String, NamedQueryParam>();
		substParams.put("0", new NamedQueryParam(DBTypes.LONG, abstractEntId));
		
		
		Collection containers = executeHQL("getContainerByAbstractEntId", substParams);
		if (containers != null && !containers.isEmpty())
		{
			container = (ContainerInterface) containers.iterator().next();
		}
