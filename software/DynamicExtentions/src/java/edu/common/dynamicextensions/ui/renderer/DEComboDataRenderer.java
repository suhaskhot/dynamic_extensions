...
public class DEComboDataRenderer {
	...
	public String render(HttpServletRequest request) throws ServletException {
		...
		ContainerInterface container = (ContainerInterface) getContainerFromUserSesion(request, containerId);
		try {
			if (container == null) {
			...
			for (ControlInterface control : container.getControlCollection()) {
				if (Long.parseLong(controlId) == control.getId()) {
					if (control.getIsSkipLogicTargetControl()) {
						...
						skipLogicEvaluationForAddMore(container, control, valueMap,
								request.getParameter(DEConstants.COMBOBOX_IDENTIFER));
				}
	...
	private Object getContainerFromUserSesion(HttpServletRequest request, String containerId) {
		Map<String, ContainerInterface> map = (Map<String, ContainerInterface>) request.getSession().getAttribute(
				WebUIManagerConstants.CONTAINER_MAP);
		ContainerInterface containerInterface = null;
		if (map != null && !map.isEmpty()) {
			containerInterface = map.get(containerId);
		} else {
			Stack<ContainerInterface> stack = (Stack<ContainerInterface>) CacheManager.getObjectFromCache(request,
	 ...
	 */
	private void executeSkipLogic(ContainerInterface container, Map<BaseAbstractAttributeInterface, Object> valueMap)
		...
		if (!container.getChildContainerCollection().isEmpty()) {
			for (ContainerInterface childContainer : container.getChildContainerCollection()) {
				SkipLogic childSkipLogic = EntityCache.getInstance().getSkipLogicByContainerIdentifier(
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void skipLogicEvaluationForAddMore(ContainerInterface container, ControlInterface control,
		...
		for (Entry<BaseAbstractAttributeInterface, Object> entry : valueMap.entrySet()) {
			if (entry.getKey() instanceof CategoryAssociationInterface) {
				CategoryEntityInterface catEntity = ((CategoryAssociationInterface) entry.getKey())
				...
				if (catEntity.getNumberOfEntries() == -1 && catEntity.equals(container.getAbstractEntity())) {
					...
					if (innerMap.size() > 0 && ControlsUtility.isControlPresentInAddMore(controlName)) {
						...
						Map<BaseAbstractAttributeInterface, Object> dataValueMap = innerMap.get(rowIndex - 1);
