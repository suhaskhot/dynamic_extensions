...
public class ControlsUtility
{
	...
	private static final String ERROR_CREATING_CONTROL_COPY = "Error creating Control copy";
	...
	*/
			...
			throws DynamicExtensionsSystemException
	{
		String sql = "select control.identifier,control.caption ,abstrCntrl.identifier,abstrCntrl.container_id, label.identifier,control.BASE_ABST_ATR_ID  from dyextn_control control left outer join  DYEXTN_LABEL label ON  control.identifier = label.IDENTIFIER left outer join dyextn_abstr_contain_ctr abstrCntrl ON control.identifier=abstrCntrl.identifier where control.container_id in ("
				+ id + ") and control.ACTIVITY_STATUS != 'Disabled'" + " order by control.SEQUENCE_NUMBER";
	...
	public static ControlInterface copy(ControlInterface control,Map<String, CategoryEntity> map) throws DynamicExtensionsSystemException
	{
		ControlInterface copyControl = null;
		try
		{
			copyControl = control.getClass().getConstructor(new Class[] {control.getClass()}).newInstance(control);
			if(control instanceof AbstractContainmentControl)
			{
				CategoryEntity target = map.get(((AbstractContainmentControl) control).getContainer().getAbstractEntity().getName());
				((AbstractContainmentControl)copyControl).setContainer((ContainerInterface) target.getContainerCollection().iterator().next());
			}
		}
		catch (IllegalArgumentException e)
		{
			throw new DynamicExtensionsSystemException(ERROR_CREATING_CONTROL_COPY,e);
		}
