...
public class TreeGenerator
{
	...
	public TreeData getTreeData(String rootName, ContainerInterface container)
	{
		...
		treedata.setImagesUrl("/images/de/");
		int counter = 0;
		...
		if (container != null)
		{
			...
			subNode = new TreeNode(containerName, counter++);
			node.add(subNode);
		}
		if (subNode == null)
		{
			node.add(new TreeNode("New Form", counter));
		}
		else
		{
			subNode.add(new TreeNode("New Form", counter));
		}
		treedata.add(node);
		return treedata;
	}
