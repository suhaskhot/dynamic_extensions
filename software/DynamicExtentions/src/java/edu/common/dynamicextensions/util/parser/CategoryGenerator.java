			...
			while (categoryFileParser.readNext())
			{
				...
				while (categoryFileParser.readNext())
				{
					...
					else
					{
						...
						final List<String> controlNotes = new LinkedList<String>();
						...
						if (heading.length() != 0)
						{
							ControlInterface headingControl = categoryHelper.addOrUpdateLabelControl(entityInterface, container,
									heading, categoryFileParser
											.getLineNumber(), controlXPosition, 0);
							headingControl.getTaggedValues().add(DomainObjectFactory.getInstance().createTaggedValue(Constants.LABEL_TYPE,Constants.HEADING));
							controlXPosition++;

						}
						if (!controlNotes.isEmpty())
						{
							for(String string:controlNotes)
							{
								ControlInterface note = categoryHelper.addOrUpdateLabelControl(entityInterface, container,
										string, categoryFileParser
												.getLineNumber(), controlXPosition, 0);
								note.getTaggedValues().add(DomainObjectFactory.getInstance().createTaggedValue(Constants.LABEL_TYPE,Constants.NOTES));
								controlXPosition++;
							}
						...
						lastControl = categoryHelper.addOrUpdateControl(entityInterface,
								...
								categoryFileParser.getControlCaption(), 
