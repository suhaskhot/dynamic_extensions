...
public final class UserInterfaceiUtility
{
	...
	public static void clearContainerStack(final HttpServletRequest request)
	{
		ContainerInterface container = (ContainerInterface) CacheManager.getObjectFromCache(
				request, UserInterfaceiUtility.getAttributeKey(request,DEConstants.CONTAINER_INTERFACE));
		if (container != null && container.getId() != null)
		{
			request.setAttribute("containerIdentifier", container.getId().toString());
		}

		CacheManager.addObjectToCache(request, UserInterfaceiUtility.getAttributeKey(request,DEConstants.CONTAINER_STACK), null);
		CacheManager.addObjectToCache(request, UserInterfaceiUtility.getAttributeKey(request,DEConstants.VALUE_MAP_STACK), null);
	...
	public static String getAttributeKey(HttpServletRequest request, String defaultKey)
	{
		if(isFormContextIdProvided(request))
		{
			return defaultKey+request.getParameter(WebUIManagerConstants.FORM_CONTEXT_IDENTIFIER);
		}
		return defaultKey;
	}

	public static boolean isFormContextIdProvided(HttpServletRequest request)
	{
		if(!"".equals(request.getParameter(WebUIManagerConstants.FORM_CONTEXT_IDENTIFIER)) && request.getParameter(WebUIManagerConstants.FORM_CONTEXT_IDENTIFIER)!= null)
		{
			return true;
		}
		return false;
