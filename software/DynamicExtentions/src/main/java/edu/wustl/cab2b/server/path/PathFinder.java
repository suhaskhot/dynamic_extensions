...
public class PathFinder
{
	...
	private Map<Long, List<IInterModelAssociation>> leftEntityVsInterModelAsso;
	...
	public List<IPath> getAllPossiblePaths(EntityInterface source, EntityInterface destination) throws DynamicExtensionsCacheException
	{
		...
		if (desEntityGroup.equals(srcEntityGroup))
		{
			...
			allPaths.addAll(getPathList(pathRecords));
		}
		else
		{
			// they are from different models
			LOGGER.info("Finding intermodel paths between : " + srcName + " and " + desName);
			allPaths.addAll(findPathsBetweenTwoModels(desEntityId, desEntityGroup, srcEntityId,
					srcEntityGroup));
		}
		return allPaths;
	}
	...
	/**
	 * This method finds path between two Entity Groups which belongs to different Models.
	 * @param desEntityId
		...
		for (InterModelConnection interModelConnection : interModelConnections)
		{
			...
			if (srcEntityGroup.equals(leftEntityGroup))
			{
				...
				if (rightEntityGroup.equals(desEntityGroup))
				{
					...
					pathList.addAll(connectPaths(pathsSrcToLeftEntity, association,
	...
	private List<PathRecord> getPathRecords(long srcId, long desId)
	{
		...
		{
			pathRecords.addAll(pathRecordCache.get(key));
		}
		return pathRecords;
	}
	...
	{
		List<IInterModelAssociation> list = leftEntityVsInterModelAsso.get(sourceEntityId);
		if (list == null)
		{
	...
	private Map<Long, IAssociation> cacheAssociationTypes(Connection con) throws DynamicExtensionsCacheException
	{
		...
		for (String[] interModelRecord : interModelRecords)
		{
			...
			if (list == null)
			{
				...
				leftEntityVsInterModelAsso.put(sourceEntityId, list);
			}
	...
	public IPath getPathForAssociations(List<IIntraModelAssociation> associations) throws DynamicExtensionsCacheException
	{
		...
		StringBuffer buff = new StringBuffer(getId(associations.get(0)).toString());
