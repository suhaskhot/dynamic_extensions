...
public class PackageName
{
	...
	private void getPackageName(final String directoryPath, final String xmiName) throws DynamicExtensionsApplicationException
	{
		...
		try
		{
			...
			if (start != -1)
			{
				packageEntityName = packageName.substring(packageName.lastIndexOf('.') + 1,
						packageName.length());

				final StringBuffer tempPackageName = new StringBuffer(packageName.substring(0,
						packageName.indexOf('.')));
				packageName = packageName.substring(packageName.indexOf('.') + 1, packageName
						.length());
			...
			throws DynamicExtensionsSystemException
	{
		...
		final Set<TaggedValueInterface> taggedValues = (Set<TaggedValueInterface>) entity
	 ...
	 */
	public static String getLogicalPackageName(final String actualPackageName,
