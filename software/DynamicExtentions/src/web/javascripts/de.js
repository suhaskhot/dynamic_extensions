...
edu.wustl.de.CategorySurveyMode = function (args) {
	...
	if (args.containerIdentifier == undefined) throw "containerIdentifier undefined";
	this.ctx = args.ctx;
	this.containerIdentifier = args.containerIdentifier;
...
edu.wustl.de.CategorySurveyMode.prototype.init = function () {
	var createpages = function (containerIdentifier, pages) {
		return function (i, e) {
			pages.push(new edu.wustl.de.Page({ctx: e, containerIdentifier: containerIdentifier,
				pageid: $(e).attr("id")}));			
	...
	$(".sm-page", this.ctx).each(createpages(this.containerIdentifier, this.pages));
};
