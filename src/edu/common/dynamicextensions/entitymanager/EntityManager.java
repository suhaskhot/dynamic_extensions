...
public class EntityManager implements EntityManagerInterface, EntityManagerConstantsInterface, EntityManagerExceptionConstantsInterface
{
			...
			DynamicExtensionsApplicationException
	{
		...
		Collection<AssociationInterface> associationCollection = executeHQL("getAssociations", substitutionParameterMap);
			...
			DynamicExtensionsApplicationException
	{
		...
		//Following method is called to execute the stored HQL , the name of which is given as the first parameter.
		//The second parameter is the map which contains the actual values that are replaced for the placeholders.
		Collection<EntityInterface> entityCollection = executeHQL("getEntitiesByConceptCode", substitutionParameterMap);
			...
			throws DynamicExtensionsApplicationException, DynamicExtensionsSystemException, DAOException, UserNotAuthorizedException
	{
		...
		{
			Collection<AbstractAttributeInterface> tempAttributeCollection = new HashSet<AbstractAttributeInterface>(attributeCollection);
	 ...
	 * @throws UserNotAuthorizedException
	 */
	private Long insertDataForSingleEntity(EntityInterface entity, Map dataValue, HibernateDAO hibernateDAO) throws DynamicExtensionsSystemException,
			DynamicExtensionsApplicationException, HibernateException, SQLException, DAOException, UserNotAuthorizedException
	{
		...
		while (uiColumnSetIter.hasNext())
		{
			...
			else
			{
				...
				if (association.getSourceRole().getAssociationsType().equals(AssociationType.CONTAINTMENT))
				{
					...
					{
						Long recordIdForContainedEntity = insertDataForSingleEntity(association.getTargetEntity(), valueMapForContainedEntity,
								hibernateDAO);
		...
		while (uiColumnSetIter.hasNext())
		{
			...
			else
			{
				...
				if (association.getSourceRole().getAssociationsType().equals(AssociationType.CONTAINTMENT))
				{
					...
					for (Map valueMapForContainedEntity : listOfMapsForContainedEntity)
					{
						Long reocordId = insertDataForSingleEntity(association.getTargetEntity(), valueMapForContainedEntity, hibernateDAO);
	 ...
	 * Map 
	 *    key    - AbstractAttributeInterface
	 *    Value  - List<String>				multiselect attribute.
	 *             
	 *             FileAttributeRecordValue	File attribute.
	 *             List<Long>				Association
	 *                  if One-One   		List will contain only 1 record id that is of target entity's record
	 *                     Many-One			List will contain one or more reocrd ids.   
	 *                                                      
			...
			DynamicExtensionsApplicationException
	{
		...
		Map<String, AttributeInterface> columnNameMap = new HashMap<String, AttributeInterface>();
		int index = 0;
	 ...
	 * @throws DynamicExtensionsApplicationException 
	 */
	private AssociationTreeObject processGroupBean(Object[] objectArray) throws DynamicExtensionsSystemException,
			DynamicExtensionsApplicationException
	{
		AssociationTreeObject associationTreeObjectForGroup = new AssociationTreeObject((Long) objectArray[0], (String) objectArray[1]);
