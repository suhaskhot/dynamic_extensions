...
public class TestEntityManagerForAssociations extends DynamicExtensionsBaseTestCase
{
	...
	public void testInheritanceMetadataSave()
	{
		...
		constraintProperties.setName("ABC");
		constraintProperties.setSourceEntityKey("SRC ENTITY KEY");
		constraintProperties.setTargetEntityKey("TARGET ENTITY KEY");
		
		
		
//		 Associate user (1)------ >(*)study       
		AssociationInterface association = new Association();
		...
		entity.setDataTableState(EntityManagerConstantsInterface.DATA_TABLE_STATE_ALREADY_PRESENT);
		try
		{
			EntityManager.getInstance().persistEntityMetadata(entity,true);
			EntityInterface savedEntity = EntityManager.getInstance().getEntityByIdentifier(entity.getId());
			
			Association association1 = (Association) savedEntity.getAssociationCollection().iterator().next();
			assertNotNull(association1.getConstraintProperties());
			assertEquals("ABC",association1.getConstraintProperties().getName());
			assertEquals("SRC ENTITY KEY",association1.getConstraintProperties().getSourceEntityKey());
			assertEquals("TARGET ENTITY KEY",association1.getConstraintProperties().getTargetEntityKey());
			
			association1.getConstraintProperties().setSourceEntityKey("Changed Source Key");
			association1.getConstraintProperties().setTargetEntityKey("Changed target Key");
			
			EntityManager.getInstance().persistEntityMetadata(savedEntity,true);
			savedEntity = EntityManager.getInstance().getEntityByIdentifier(entity.getId());
			Association association2 = (Association) savedEntity.getAssociationCollection().iterator().next();
						
			assertNotNull(association2.getConstraintProperties());
			assertEquals("ABC",association2.getConstraintProperties().getName());
			assertEquals("Changed Source Key",association2.getConstraintProperties().getSourceEntityKey());
			assertEquals("Changed target Key",association2.getConstraintProperties().getTargetEntityKey());
		}
		catch (Exception e)
		{
			// TODO Auto-generated catch block
